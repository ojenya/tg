...
bot.start((ctx) => {
    ctx.reply(
        'Введите фамилию, имя и отчество', { reply_markup: { remove_keyboard: true } }
    )
...
getName.command('start', async(ctx) => {
    ctx.reply(
        'Начнем заново. Введите имя, фамилию и отчество', { reply_markup: { remove_keyboard: true } }
    )
...
check.hears('️✅ Все верно', (ctx) => {
    ...
    for (let key of data.admins) {
        bot.telegram.sendMessage(
            ...
            `\nТема диплома: ${ctx.session.theme};\nЯзыки: ${ctx.session.langs};\nВладение компьютером: ${ctx.session.compSkills};` +
            `\nНомер: ${ctx.session.phone}`, { parse_mode: 'markdown' }
        )
    }
    ctx.session = null
})
